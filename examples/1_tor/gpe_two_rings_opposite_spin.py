...
# ==========================================
# 2. Начальное условие: Два кольца с ПРОТИВОПОЛОЖНЫМ вращением
# ==========================================
z1 = -5.0
z2 = 5.0
...
# Вращение ПО ЧАСОВОЙ (m = 1)
psi_1 = np.exp(-0.5 * ((r_cyl - R_torus)**2 + (z - z1)**2)) * np.exp(1j * 1 * theta) * np.exp(1j * kz * z)

# Кольцо 2 (сверху): летит вниз (-kz)
# Вращение ПРОТИВ ЧАСОВОЙ (m = -1). 
# Знак минус перед выражением оставляем для жесткого отскока (темный солитон в центре).
